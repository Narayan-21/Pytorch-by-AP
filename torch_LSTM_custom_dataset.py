import os # When loading filepaths
import pandas as pd
import spacy
import torch
...
class Vocabulary:
    def __init__(self, freq_threshold):
        self.itos = {0:'<PAD>', 1:'<SOS>', 2:'<EOS>', 3:'<UNK>'} # PAD token is used to keep the length of your each batch to be the same.
        self.stoi = {'<PAD>': 0, '<SOS>':1, '<EOS>':2, '<UNK>':3} #stoi-> string to index
...
class FlickerDataset(Dataset):
    def __init__(self, root_dir, captions_file, transform=None, freq_threshold=5):
        ...
        # Get image and captions columns
    ...
    def __getitem__(self, index):
        ...
        numericalized_caption.append(self.vocab.stoi["<EOS>"]) # End-of-sentence token
...
class MyCollate:
    ...
    def __call__(self, batch):
        ...
        targets = [item[1] for item in batch] # list of captions
